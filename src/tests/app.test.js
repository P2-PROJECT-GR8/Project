...
describe("newtuple", () => {
  it("when recieveing objectid, relation, subjectid it should create a new tuple in the db", async () => {
    ...
    expect(fakeDb.data.tupleStore.bySubject).toEqual({
      "user:testuser": [{ relation: "owner", objectId: "file:testfile" }],
    });
...
describe("saveAllChanges", async () => {
  it("adds relations", async () => {
    ...
    expect(spy).toHaveBeenCalledWith("user:testuser", "owner", "file:testfile");
  ...
  it("deletes a relation", async () => {
    ...
    fakeDb.data.tupleStore.byObject["file:testfile"].push({
      relation: "owner",
      subjectId: "user:testuser",
    });
  ...
  it("updates a relation if given proper arguemtns", async () => {
    ...
    fakeDb.data.tupleStore.bySubject["user:testuser2"].push({
      relation: "viewer",
      objectId: "file:testfile",
    });
    fakeDb.data.tupleStore.byObject["file:testfile"].push({
  ...
  it("returns status 403 if unauthorized", async () => {
    // initial db state
    fakeDb.data.users.push({ id: "user:testuser", name: "testuser" });
...
describe("foldercontent", async () => {
  ...
  it("retuns a lsit of all files / folders and actions within a folder if given folderid", async () => {
    ...
    fakeDb.data.tupleStore.byObject["folder:testfolder"].push({
      relation: "owner",
      subjectId: "user:testuser",
    });

    fakeDb.data.tupleStore.bySubject["folder:testfolder"].push({
      relation: "parent",
      objectId: "file:testfile",
    });
    fakeDb.data.tupleStore.byObject["file:testfile"].push({
    ...
    const res = await request(app)
      .get("/api/folderContent?folderId=folder:testfolder")
      .set("Cookie", `sessionToken=${token}`);
  ...
  it("return a status 403 with messeage No access to this folder if folder cant be found", async () => {
    ...
    const res = await request(app)
      .get("/api/folderContent?folderId=folder:doesntexist")
      .set("Cookie", `sessionToken=${token}`);
  ...
  it("does not include duplictae relations for the same object", async () => {
    ...
    expect(res.body.files[0].relations).toEqual(
...
describe("creatnew", async () => {
  it("creates a file with the currenjt user as owner if not folder is prvided", async () => {
    ...
    expect(fakeDb.data.tupleStore.byObject).toEqual({
      "file:testfile": [{ relation: "owner", subjectId: "user:testuser" }],
    });
  });
  it("creates a file under a folder if a folder id is provided", async () => {
    ...
    expect(
      fakeDb.data.tupleStore.bySubject["folder:testfolder"],
    ).toContainEqual({ relation: "parent", objectId: "file:testfile" });
    expect(fakeDb.data.tupleStore.byObject["file:testfile"]).toContainEqual({
      relation: "parent",
      subjectId: "folder:testfolder",
    });
  });
...
describe("adminFiles", async () => {
  it("should return a list of all files for provided userid", async () => {
    ...
    const res = await request(app)
      .get("/api/adminFiles?userId=user:testuser")
      .set("Cookie", `sessionToken=${token}`);

    expect(res.status).toBe(200);
    expect(res.body.files).toEqual([
      { objectId: "file:testfile", relations: ["owner"] },
    ]);
  ...
  it("should disallow any user that isnt admin from making a request", async () => {
    // initial db state
    fakeDb.data.users.push({ id: "user:testuser", name: "testuser" });
    fakeDb.data.users.push({ id: "user:admin", name: "admin" });
